package com.deecoders.meribindiya.network;
...
import com.deecoders.meribindiya.util.InterenetReceiver;
...
public class VolleyLibrary {
    ...
    public <T> void addToRequestQueue(Request<T> req, String tag, boolean retry) {
        if(InterenetReceiver.isNetworkAvailable(mCtx)){
    ...
    public <T> void addToRequestQueueWithoutInternetCheck(Request<T> req, String tag, boolean retry) {
        req.setTag(tag);
        if(retry){
            req.setRetryPolicy(new DefaultRetryPolicy(10 * 1000, 2, 1.0f));
        }
